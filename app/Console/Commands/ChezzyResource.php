...
class ChezzyResource extends Command
{
    ...
    public function handle(): void
    {
        ...
        // PERBAIKAN: Gunakan array_values untuk mereset index kembali ke 0, 1, 2, dst.
        ...
        // PERBAIKAN: Reset index lagi setelah dikurangi excluded models
        ...
        // PERBAIKAN: Cek jika total 0 agar tidak terjadi Division by Zero di progress bar
        if ($totalModels === 0) {
        ...
        foreach ($modelsToProcess as $model) {
            // PERBAIKAN: Gunakan $selectedOption dari input user, bukan di-hardcode 'default'
    ...
    private function askExcludedModels(array $models): array
    {
        ...
        while (true) {
            ...
            // PERBAIKAN: Trim input untuk membersihkan spasi
            ...
            // PERBAIKAN: Cek identik strict dengan string kosong ('') agar angka '0' tidak dianggap empty
            if ($input === '') {
                break;
            }
    ...
    {
        // PERBAIKAN: Menggunakan flag murni dari Filament (jika versi mendukung) atau menjalankan exec
        // dengan lebih aman. Jika Anda butuh bypass pertanyaan "title attribute",
        // lebih aman biarkan Filament menggunakan default ('id') dengan opsi --no-interaction
        // agar terminal tidak hang.
        ...
        // Menggunakan Facade Process bawaan Laravel (lebih clean dari proc_open)
        Process::run($command);
    }
